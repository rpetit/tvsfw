...
path = '/home/petit/Documents/these/code/python/tvsfw/experiments/exp2/figures/'

points1 = np.array([[-0.6, 0.85], [-0.75, 0.65], [-0.65, 0.31], [-0.75, 0.0], [-0.5, -0.5],
                    [-0.15, 0.15], [0.1, 0.5], [-0.25, 0.85]]) + np.array([-0.1, -0.1])

points2 = 0.6 * np.array([[-0.2, 0.75], [-0.3, 0.5], [-0.2, 0.3], [0.25, 0.45], [0.3, 0.6], [0.25, 0.8]]) + np.array([-0.45, 0.0])

points3 = np.array([[0.5, 0.25], [0.35, 0.1], [0.3, -0.1], [0.15, -0.3], [0.5, -0.5], [0.8, 0.0]]) + np.array([0.0, -0.1])

vals = np.array([1.0, -2, 1.5])
...
alpha = 0.25 * sqrt(2 * log(y.size)) * std_noise
...
for iter in range(3):
    ...
    E, obj_tab, grad_norm_tab = compute_cheeger(eta,
                                                max_tri_area_fm=1e-3, max_iter_fm=10000, plot_results_fm=False,
                                                num_boundary_vertices_ld=50, max_tri_area_ld=1e-2,
                                                step_size_ld=1e-2, max_iter_ld=200, convergence_tol_ld=1e-6,
                                                num_iter_resampling_ld=60, plot_results_ld=False)

    u_hat.extend_support(E)
    u_hat.fit_weights(noisy_y, phi, alpha)

    obj_tab, grad_norm_tab = u_hat.perform_sliding(noisy_y, phi, alpha, 0.5, 200, 0.05, 0.5, 1e-8, 60, 25, 0.004)
    ...
    plot_simple_function(u_hat, m, save_path=path+'estim_{}.png'.format(iter))
    plot_simple_function(simple_function_diff(u, u_hat), m, save_path=path+'diff_{}.png'.format(iter))
    plot_supports(u, u_hat, save_path=path+'diff_support_{}.png'.format(iter))
